...
void init_thread()
{
    // nettoie l'array avant toute action, lorsqu'il est à zero
    if (store_user.index == 0) {
        int index = 0;
        while (index < 3) {
            /* détruit */
            if (store_user.thread_clients[index] != 0)
                pthread_detach(store_user.thread_clients[index]);
            /* initialise à zero efface */
            store_user.thread_clients[index] = 0;
            index++;
        }
    }
