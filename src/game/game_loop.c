...
int area_button(control_t *control, SDL_Event event)
{
	if (event.motion.y >= (WINDOW_HEIGHT - 64) && event.motion.y <= (WINDOW_HEIGHT - 4) && event.button.button == SDL_BUTTON_LEFT)
	{
		if (event.motion.x >= 150 && event.motion.x <= 300)
		{
			if (control->send_state == 1)
				prepare_client_request(control, control->network->client_id, 12, 12, 0);
			if (control->network->net_clt_srv_state == 1)
			{
				prepare_client_request(control, control->network->client_id, 50, 0, 0);
...
void game_loop(control_t *control, int *menuSelect)
{
		...
		if (SDL_PollEvent(&event))
		{
			switch (event.type)
			{
				...
				case SDL_KEYDOWN:
					...
					{
						case SDLK_LEFT:
							prepare_client_request(control, control->network->client_id, 2, 2, 0);
							break;
						case SDLK_RIGHT:
							prepare_client_request(control, control->network->client_id, 2, 2, 1);
							break;
						case SDLK_UP:
							prepare_client_request(control, control->network->client_id, 2, 3, 0);
							break;
						case SDLK_DOWN:
							prepare_client_request(control, control->network->client_id, 2, 3, 1);
							break;
						case SDLK_b:
							prepare_client_request(control, control->network->client_id, 2, 4, -1);
							break;
						case SDLK_s:
							prepare_client_request(control, control->network->client_id, 2, 5, -1);
							break;
						default:
							break;
					}
