...
//void *server_receive_from_client(void *tmp) {
    ...
    while (((read_size = recv(sock, (char *) requete, sizeof(t_clt_sd), 0)) != SOCKET_ERROR)) {
...
//
        if (requete->commandservice == 1)
            ...
            shutdown((int) etat->sock_tmp, 2);
        else
            dispatchRequete(etat, requete);
        etat->flage_stop = 1; /* signal un nouvel user */
       // pthread_cond_signal(&cv);  // MODIF AJOUT MUTEXT ET  pthread_cond_signal
       // printf("hello %u\n", (unsigned int)etat->sock_tmp);
    }
    if (read_size == -1)
        perror("recv failed");
    puts("client disconnet\n");
    printf("je suis deconnecte");
  //  pthread_exit(NULL);
  //  return (1);
}
...
//        if (FD_ISSET(etat->sock_server, &readfds)) {
            ...
            if ((client_sock = accept(socket, (struct sockaddr *) &client, (socklen_t *) &c))) {
              ...
              //  int rc = 0;
              //  SDL_Thread *thread;
                etat->sock_tmp = (SOCKET) client_sock;
                //server_receive_from_client((void *)etat);

             /*   if(SDL_CreateThread(server_receive_from_client, "server_receive_from_client", (void *)etat) == NULL)
                {
                    opt = 0; // désactive le serveur
               ...
               // rc = pthread_detach(store_user.thread_clients[store_user.index]);
              //  store_user.index++;
            }
        }
    }
