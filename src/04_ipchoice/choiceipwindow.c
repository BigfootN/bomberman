...
void write_text(t_control* control)
{
    ...
    SDL_Rect render_quad = {200, 298, my_strlen(control->network->ip_serveur) * 25, 60};
    write_string(control, render_quad, control->network->ip_serveur);
...
char * delete_data(t_control* control, SDL_Event event)
{
    ...
    if ((event.key.keysym.scancode == SDL_SCANCODE_BACKSPACE) && index)
    {
        index -= 1;
        tmp[index] = '\0';
        control->network->ip_serveur = tmp;
    }
...
char *write_data(SDL_Event event, char *tmp)
{
    ...
    if (my_choice_number(lettre[0]) && index >= 0 && index < 15)
    {
        lettre = lettre + '\0';
        tmp[index] = *lettre;
        index += 1;
