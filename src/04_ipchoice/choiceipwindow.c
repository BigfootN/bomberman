#include "headers.h"

void createEtiquettes(t_control *control)
{
    SDL_Rect renderQuad = {200, 200, 385, 60};
    writeString(control, renderQuad, "adresse ip serveur");
    // TTF_Font *font = NULL;

    //  TTF_Init();
    // SDL_Color colorNoire = {0, 0, 0, 255};
    /* Chargement de la font */
    // font = TTF_OpenFont("roboto/Roboto-Black.ttf", 45);
    // if(font == NULL)
    //     SDL_Quit();
    /*
        SDL_Surface* textSurface = TTF_RenderText_Solid(control->sprites->font, "adresse ip serveur", control->sprites->color);
        SDL_Texture* text = SDL_CreateTextureFromSurface(control->main_renderer, textSurface);
        SDL_FreeSurface(textSurface);
        SDL_Rect renderQuad = {200, 200, 385, 60};
        SDL_RenderCopy(control->main_renderer, text, NULL, &renderQuad);
        SDL_DestroyTexture(text);*/
    //  TTF_Quit();*
}

void writeText(t_control* control)
{
    //SDL_Color colorNoire = {0, 0, 0, 255};
    //TTF_Font *font = NULL;

    //TTF_Init();

    SDL_Rect renderQuad = {200, 298, my_strlen(control->network->sever_addr) * 25, 60};
    writeString(control, renderQuad, control->network->sever_addr);

    //font = TTF_OpenFont("roboto/Roboto-Black.ttf", 50);
    //SDL_Color textColor = {255, 255, 255, 0};
    // SDL_Surface* textSurface = TTF_RenderText_Solid(font, control->network->sever_addr, colorNoire);
    //SDL_Texture* text = SDL_CreateTextureFromSurface(control->main_renderer, textSurface);
    // SDL_FreeSurface(textSurface);
    //SDL_Rect renderQuad = {200, 298, 385, 60};
    // SDL_Rect renderQuad = {200, 298, my_strlen(control->network->sever_addr) * 25, 60};
    // SDL_RenderCopy(control->main_renderer, text, NULL, &renderQuad);
    // SDL_DestroyTexture(text);
    // TTF_Quit();
}

int my_choicenumber(int c)
{
    if ((c >= 48 && c <= 57) || (c == 46))
        return (1);
    return (0);
}

char * deleteData(t_control* control, SDL_Event event)
{
    int index;
    char *tmp;
    tmp = (char*) malloc(sizeof (char*)*16);
    bzero(tmp, 16);
    // event.key.keysym.scancode == SDLK_BACKSPACE ||
    tmp = control->network->sever_addr;
    index = my_strlen(tmp);
    if ((event.key.keysym.scancode == SDL_SCANCODE_BACKSPACE) && index)
    {
        index -= 1;
        tmp[index] = '\0';
        control->network->sever_addr = tmp;
    }
    encartClair(control);
    return tmp;
}

char *writeData(SDL_Event event, char *tmp)
{
    char *lettre;
    int index;


    index = my_strlen(tmp);
    lettre = (char*) malloc(sizeof (char*));
    lettre = event.text.text;

    if (my_choicenumber(lettre[0]) && index >= 0 && index < 15)
    {
        lettre = lettre + '\0';
        tmp[index] = *lettre;
        index += 1;
    }
    tmp[index] = '\0';
    return tmp;
}