...
void prepa_bomb_map(t_etat* etat)
{
    ...
    while (indexX < etat->data_map->pos.x)
    {
        ...
        while (indexY < etat->data_map->pos.y)
        {
            ...
            {
                etat->data_map->bmmap[indexX][indexY] += 1; /* on detruit definitevement l'exlosion*/
                ...
                if (0 <= indexY && (etat->data_map->pos.y) > indexY)
                {
                    ...
                    if (mur == 19)
                    {
                        ...
                        if (sort >= 27 && sort <= 30)
                        {
                            ...
                            position.x = indexX;
                            position.y = indexY;
                            t_pions *pion = add_item_pions(etat,  position); // pour la suppression chercher aussi les emplacements x y lorsque un est trouvé
                            pion->active = 1;
                            pion->life = 5;
                            pion->etat_requete = 1;
                            if (sort >= 27 && sort <= 30)
        ...
        while (indexY < 5)
        {
            if (tab[indexX][indexY] != 1)
            {
                ...
                {// couloirs vides
                    ...
                    {
                        int player = etat->data_map->mapaction[row][col]; // supprimer immediatement
                        etat->data_map->mapaction[row][col] = flag; /* on mets l'explosion */
                        desactive_players(etat, player); /* supprime immediatement du jeux */
                    }
