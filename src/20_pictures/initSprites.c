...
int central_init_sprites(t_control* control) {
	if (search_folder(REP_SPRITES, control) &&
		search_folder(REP_TTF, control) &&
		search_folder(REP_MAP, control) == 1) {
...
char* create_directory_file(char const* name, int directory) {
	...
	size_t prefix_size;
	size_t total_size;
	const size_t name_size = my_strlen(name);
	...
	prefix_size = my_strlen(prefix);
	total_size = prefix_size + name_size;

	file_path = (char*) malloc(sizeof(*file_path) * total_size + 1);
	memcpy(file_path, prefix, sizeof(*prefix) * prefix_size);
	memcpy(file_path + prefix_size, name, sizeof(*name) * name_size);
	file_path[total_size] = '\0';
...
int init_attente(t_control* control) {
	...
	name_image = create_directory_file("attente1.png", 1);
...
int init_panneau_resultat(t_control* control) {
	...
	name_image = create_directory_file("panneau_final.png", 1);
